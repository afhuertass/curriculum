...
local_maxi = peak_local_max(distance, indices=False, footprint=np.ones((15, 15)), labels=mask2)
...
imgbigper = ""
for prop in props:
    if prop.perimeter > perimetrobig:
        perimetrobig = prop.perimeter
        imgbigper = prop.image
    if prop.area > areabig :
        areabig = prop.area
        coordbig = prop.coords
        imgbig = prop.image
    areatotal = areatotal + prop.area
    perimetro = perimetro + prop.perimeter
...
print("Perimetro  Total - Watersheed:")
print(perimetro*px_um)

print("Elemento mayor Perimetro - Watersheed:")
print(perimetrobig*px_um)
