...
double Cuerpo::GetAngleX(double beta){
...
int main(){
  ...
  cout << beta << " " << (0*Planeta[0].GetAngleX(beta) + Planeta[1].GetAngleX(beta))*(180.0/M_PI) << endl ;
  ...
  for(t=tdibujo=0;t<T;t+=Deltat,tdibujo+=Deltat){
    ...
    /*if( Planeta[0].GetColision() == false ){
      cout << beta << " " << (Planeta[0].GetAngleX(beta) + Planeta[1].GetAngleX(beta))*(180.0/M_PI) << endl ;
      break;
    }
    }
    */
