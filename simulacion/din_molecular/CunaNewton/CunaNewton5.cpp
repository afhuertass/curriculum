...
public:
  ...
  void CalculaTorqueQueMeHace(Cuerpo ElOtroPendulo);      
...
void CalculaTorqueQueMeHace(Cuerpo ElOtroPendulo){
  double h=Getx()-ElOtroPendulo.Getx();
  double h=(R+ElOtroPendulo.R)-fabs(dx);
  if (h>0) N+=dx/fabs(dx*k*pow(h,1.5));
}    
...
int main(){
  ...
  for(t=tdibujo=0;t<3*T;t+=Deltat,tdibujo+=Deltat){
    ...
    for(i=0; i<N; i++) {Pendulo[i].CalculaTorqueQueMeHace(Pendulo[i+1]);}
    for(i=1; i<N; i++) {Pendulo[i].CalculaTorqueQueMeHace(Pendulo[i-1]);}
